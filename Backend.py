...
def convert_to_rdf(document_structure, tables, all_images_data, base_image_url):
    ...
    g.add((EX.ContentPart, RDF.type, OWL.Class))
    ...
    g.add((EX.hasContent, RDF.type, OWL.ObjectProperty))
    ...
    for sec_index, section_obj in enumerate(document_structure.get("sections", [])):
        ...
        for content_item in section_obj.get("content", []):
            ...
            for i, chunk in enumerate(chunks):
                part_uri = URIRef(f"{str(section_instance_uri)}_ContentPart_{i + 1}")
                g.add((part_uri, RDF.type, EX.ContentPart))
                g.add((part_uri, RDFS.label, Literal(f"Content-{i + 1}")))
                g.add((part_uri, RDF.value, Literal(safe_literal(chunk))))
                g.add((section_instance_uri, EX.hasContent, part_uri))
        ...
        for sub_sec_index, sub_section_obj in enumerate(section_obj.get("subsections", [])):
            ...
            for content_item in sub_section_obj.get("content", []):
                chunks = chunk_content(content_item)
                for i, chunk in enumerate(chunks):
                    part_uri = URIRef(f"{str(subsection_instance_uri)}_ContentPart_{i + 1}")
                    g.add((part_uri, RDF.type, EX.ContentPart))
                    g.add((part_uri, RDFS.label, Literal(f"Content-{i + 1}")))
                    g.add((part_uri, RDF.value, Literal(safe_literal(chunk))))
                    g.add((subsection_instance_uri, EX.hasContent, part_uri))
